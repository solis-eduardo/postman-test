# Visão geral: como Postman e Fern se integram
...
- `docs/TROUBLESHOOTING.md` — tabela consolidada de todo erro já enfrentado.
...
flowchart LR
    ...
    SPEC --> COLLECTION["postman/collections/COLLECTION_NAME/<br/>(collection v3, Native Git)"]
    SPEC --> FERN["fern/generators.yml<br/>(Fern Docs)"]
    SPEC --> REDOC["docs-pages.yml<br/>(portal Redoc)"]

    COLLECTION --> MOCK["postman/mocks/MOCK_SLUG/<br/>(mock local)"]
    ...
    FERN --> SITE1["FERN_DOMAIN"]
    REDOC --> SITE2["GitHub Pages"]
```
...
| `postman/collections/<COLLECTION_NAME>/` | Requests, exemplos, scripts de teste | Postman Desktop (Native Git) lê/escreve; `scripts/export_v2_collection.py` lê pra gerar `dist/` | Sim, mas pra **comportamento observável** (exemplos, testes) — não pro contrato formal |
...
| `postman/mocks/<MOCK_SLUG>/config.yaml` | Porta/protocolo do mock local | Editado pelo Postman Desktop (UI) | Sim, só pra "em que porta o mock escuta" — **precisa bater manualmente** com `base_url` do ambiente Local (Postman não sincroniza os dois; ver `docs/postman-git-native.md`) |
| `postman/mocks/<MOCK_SLUG>/default.js` | Respostas do mock por rota | Gerado pelo Postman a partir dos exemplos salvos; editado à mão quando falta exemplo | Não — é derivado dos exemplos da collection |
...
| `.postman/resources.yaml` | A qual workspace/organização Postman este repo está ligado; quais arquivos fora de `postman/` são registrados | Escrito pelo Postman Desktop ao conectar; editado à mão quando precisa registrar algo (ex.: a spec) | Sim, pro vínculo repo↔workspace |
| `postman-fern.env` | Nomes/slugs usados por todos os scripts e workflows deste kit | Editado à mão (uma vez, na adoção) | Sim, é a config central do kit |
...
| `fern/docs.yml` | Domínio, título, navegação, tema (cores light/dark), logo, navbar-links | Editado à mão | Sim, pra aparência/URL do site |
...
| `scripts/check_spec_alignment.py` | Confere spec ↔ collection (endpoints + tags + schema) | — | N/A (é a checagem, não a fonte) |
...
do projeto. Editar pela UI do app (criar request, mudar variável de
...
- **Mock ↔ ambiente Local**: a porta do mock (`config.yaml`) e a `base_url`
  ...
  (`api-tests.yml`) falha alto se isso acontecer, lendo as duas e
...
3. Publicar de verdade (`fern generate --docs`) é **manual ou via CI**
   ...
   desse passo rodar.
4. O site publicado só reflete a mudança depois que esse passo 3 termina.
...
uma pasta na UI do Postman não muda nada no site publicado; quem controla o
agrupamento/navegação da Fern são as `tags` de `postman/specs/openapi.yaml`.
É o mesmo padrão da tabela acima: cada sistema lê uma fonte específica, nada
sincroniza sozinho entre eles.
...
GitHub — foi assim que uma correção em `fern.config.json` chegou a acontecer
num projeto real sem ninguém rodar `git commit` manualmente. Se você usar o
Dashboard da Fern pra ajustar algo (tema, logo etc.), espere um commit
aparecer no histórico — não é bug, é o funcionamento esperado.
...
2. Editar a requisição correspondente em
   `postman/collections/<COLLECTION_NAME>/**/*.request.yaml` (pela UI do
   Postman ou à mão) pra bater com o novo contrato — `scripts/check_spec_alignment.py`
...
4. `npm run export:v2` só é necessário rodar manualmente se for
   ...
   por link, mock duplicado etc.); em push direto na `main`, o CI regenera e
   commita `dist/` sozinho se precisar.
...
6. Nenhum desses passos edita a **spec** de volta — ela é sempre a origem,
   nunca o destino, de qualquer sincronização automática deste kit.
